...
def sim(events, noise, total_events, sp_density, t_density, mix=False, verbose = False, eventscale=1, spacesigma=0.00021233045007200478, start_time=-1, start_x=-1, start_y=-1, dataSaveID = None, file=None):
    ...
    for i in range(events): # each evet 
        
        label = random.randint(1, 10000) # generate label and source 
        ...
        event = []
        for j in range(num_photons): # generate coords for each photon
            coords = [0]*3
        ...
        event.sort(key = lambda coord: coord[2])
        master_data.append(event) 
    ...
    return np.array(data), labels, sources 
